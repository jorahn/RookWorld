...
directories = {
    ("classic", "10B"): ("fineweb10B_rookworld", "sample-10BT"),
    ("classic", "100B"): ("fineweb100B_rookworld", "sample-100BT"),
    ("edu", "10B"): ("edu_fineweb10B_rookworld", "sample-10BT"),
    ("edu", "100B"): ("edu_fineweb100B_rookworld", "sample-100BT")
}
...
if args.type == "classic":
    ...
    name = "fineweb_rookworld"
elif args.type =="edu":
    fw = load_dataset("HuggingFaceFW/fineweb-edu", name=remote_name, split="train")
    name = "edu_fineweb_rookworld"

rw = load_dataset("jrahn/rookworld_7m", split="train")

len_fw = len(fw)
len_rw = len(rw)
len_total = len_fw + len_rw

# interleaving the two datasets
ds = interleave_datasets([fw, rw], probabilities=[len_fw/len_total, len_rw/len_total])

print(f"Dataset proportions:")
print(f"FineWeb: {len_fw:,} ({len_fw/len_total:.2%})")
print(f"RookWorld: {len_rw:,} ({len_rw/len_total:.2%})")
print(f"Total documents: {len(ds):,}")

# Dataset proportions:
# FineWeb: 9,672,101 (58.14%) (longer documents)
# RookWorld: 6,963,413 (41.86%) (shorter documents)
# Total documents: 16,634,664

# edu_fineweb10b train/val *.bin files storage: 19GB (19,441,756)
# edu_fineweb10b_rookworld train/val *.bin files storage: 21G (21,460,436)
# added +10% tokens for rookworld dataset
